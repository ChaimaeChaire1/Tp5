...
router.get('/Chefs/:id',verfyToken, AsyncHandler(
    async (req, res) => {
        const chefstId = req.params.id;
        try {
            const response = await axios.get(`http://localhost:3001/Chef/chefs/${chefstId}`);
...
router.get('/recettes/:id',verfyToken, AsyncHandler(
    async (req, res) => {
        ...
        try {
            const response = await axios.get(`http://localhost:3002/recettes/recette/${restaurantId}`);
            res.status(200).json(response.data);
        } catch (error) {
            console.error('Erreur lors de la récupération des recettes:', error);
            res.status(500).json({ message: 'Erreur lors de la récupération des recettes' });
